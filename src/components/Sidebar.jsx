...
export default function Sidebar({ messages, viewingAs, partnerId, className }) {
  ...
  return (
    <aside
      className={cn(
        'w-full min-w-0 shrink-0 flex-col border-r border-white/5 bg-tm-panel md:w-[340px] lg:w-[380px]',
        className,
      )}
    >
      <div className="flex items-center justify-between px-4 py-3">
        <h2 className="text-lg font-semibold text-tm-text">Chats</h2>
        <div className="flex items-center gap-1 text-tm-muted">
          <button className="grid h-9 w-9 place-items-center rounded-full transition hover:bg-tm-panel-2 hover:text-tm-text">
      ...
      <div className="px-3 pb-2">
        <div className="flex items-center gap-3 rounded-lg bg-tm-panel-2 px-3.5 py-2">
          <Search className="h-4 w-4 text-tm-muted" />
          <input
            placeholder="Search or start a new chat"
            className="w-full bg-transparent text-[13px] text-tm-text placeholder:text-tm-muted/70 focus:outline-none"
          />
        </div>
      </div>

      <div className="flex-1 overflow-y-auto">
        <button className="flex w-full items-center gap-3 border-b border-white/5 bg-tm-panel-2/40 px-4 py-3 text-left">
          <Avatar user={partner} size="lg" online />
          <span className="min-w-0 flex-1">
            <span className="flex items-baseline justify-between gap-2">
              <span className="truncate text-[15px] font-medium text-tm-text">{partner.name}</span>
            ...
            <span className="mt-0.5 flex items-center justify-between gap-2">
              ...
              {unread > 0 && (
                <span className="grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-tm-rose px-1.5 text-[11px] font-semibold text-tm-bg">
                  {unread}
                </span>
              )}
            </span>
        ...
        <div className="px-4 py-6 text-center">
          <Archive className="mx-auto mb-2 h-5 w-5 text-tm-muted/50" />
